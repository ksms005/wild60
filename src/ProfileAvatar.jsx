...
export default function ProfileAvatar({person,size='md',editable=false,companyId,onUpdated,className=''}){
 ...
 return <div className={`profileAvatarWrap ${className}`} data-size={size}><div className="profileAvatar">{person?.avatar_url?<img src={person.avatar_url} alt={`${person.full_name||'Team member'} profile`}/>:<span>{initials(person?.full_name)}</span>}{editable&&<button type="button" className="avatarEditBtn" title="Add or change profile photo" onClick={()=>input.current?.click()} disabled={busy}>{busy?<LoaderCircle className="avatarSpinner"/>:<Camera/>}</button>}</div>{editable&&<input ref={input} className="avatarFileInput" type="file" accept="image/jpeg,image/png,image/webp,image/gif" onChange={choose}/>} {error&&<small className="avatarError">{error}</small>}</div>
}
